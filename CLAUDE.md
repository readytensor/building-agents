...
- **Episode arc (code additions):**
  ...
  - Ep 4 — plan step (TODO-style tool: `write_plan`) + in-the-moment reasoning step (`think`, no-op echo). Reflection was tried and dropped (false positives dominated; no real spirals caught). Done tool already lives in Ep 3.
...
- **Ep 4** — implemented + 1 baseline run + 1 planning run recorded + producer brief done. Agent.py ~330 LOC (Ep 3 base + `write_plan`/`CURRENT_PLAN` + `think` tool, plus the plan-in-system prompt mechanism). Task: **add reference-style markdown links to `md2html`** — a feature add, not a bug fix; touches lexer + parser + renderer + extension registry. Headline empirical finding: **planning + think made the agent more expensive, not cheaper** (Ep 3 baseline 27 iter / ~$0.61; Ep 4 with planning 47 iter / ~$0.91, +49% cost). Reframe: planning is a *legibility tax*, not a speedup — useful when you're paying to know what the agent is doing, not to make it faster. Side findings: (a) model used `write_plan` exactly ONCE (set up plan at start, never updated it — plan-as-decoration); (b) `think` used liberally (9 calls / 47 iters, like a scratchpad); (c) **`done()` reliability still broken in both runs** — neither baseline nor planning agent called done(), both exited via naive stop. The structured plan makes the gap *visible* (unfinished steps next to "I'm done" text). That's the bridge to Ep 5. Producer brief at `tmp/video-creation-notes/episode-04.md`. **Locked with caveats** — see "Ep 4 carryover gotchas" below.
- **Ep 5** — not yet spec'd. Multi-agent + delegate per the locked arc.

## Ep 4 carryover gotchas (read before starting Ep 5 work)

1. **Dev-time SDK swap.** Ep 4's `agent.py` and `_before.py` temporarily use the **native Anthropic SDK** (not the locked `openai` package) to access prompt caching during development. **Must translate back to the openai SDK shape before publishing** the companion code. Ep 5's agent.py should follow the same dev-time pattern (native Anthropic for development, translate back before ship). Header docstrings in Ep 4's agent.py explain.
2. **Prompt caching is fragile to message-prefix mutations.** A naive "inject dynamic state (plan, scratchpad, etc.) into the last user message of each call" pattern silently breaks the cache — the prefix hash shifts every iter because the "last user message" keeps moving. Anthropic's lookback is only 20 blocks, and even within that window, the changed content invalidates the entry. **Rule:** dynamic per-call state goes in the **system prompt** (rebuilt as `[stable base text with cache_control, dynamic addendum without]`), NOT in messages. Tested and confirmed in Ep 4's `_check_prefix_stability` instrumentation (preserved in commit `9a6e4f2`, stripped in `fc0b752`). Same rule will apply to anything Ep 5 wants to expose between agents.
3. **`done()` reliability is now overdue.** Three of the last three episodes' recorded runs ended via naive stop, not `done()`. With multi-agent, the question "who decides we're done" is sharper — Ep 5 is the natural place to either solve it (e.g., orchestrator agent only) or explicitly stop pretending it's solved.
4. **`spec/episode-04.md` had to be reconciled after the fact.** Pre-implementation it described a tables-in-lists bug-fix task and a reflection-loop tool. Implementation pivoted to reference-style-links feature-add + `think` (reflection dropped after trial runs produced false positives without catching real spirals). Spec is now aligned. For Ep 5: write the spec close enough to implementation that this kind of post-hoc reconcile isn't needed again.
5. **Feature-add proved harder than bug-fix** for revealing failure modes. Ep 4 baseline on this task: 27 iter (vs Ep 2's 17 for the parser bug fix). For Ep 5: pick a task whose natural shape makes multi-agent's value plausible *a priori* — likely a task with independently-parallelizable subtasks (e.g., implement two unrelated markdown features at once, where one agent could spec/coordinate and another could implement).
