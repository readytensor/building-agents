...
This episode's headline mechanism: rolling-summary compaction. When a single
LLM call's input grows past COMPACTION_THRESHOLD, the older middle of the
message history is summarized via a second LLM call and replaced with one
summary message — so a long-running task doesn't keep paying for the full
